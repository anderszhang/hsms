...
module.exports = (function() {
  ...
  class DataItem {
    constructor(builder) {
      if (
        arguments.length < 1 ||
        !(String(builder.constructor) === String(DataItem.builder.constructor))
      ) {
     ...
     */
    toString(indent = "") {
      const key = Object.keys(ItemFormat).find(
    ...
    static numeric(f = ItemFormat.I2, name = "", ...values) {
      return DataItem.builder
        .format(f)
        .value(
          0 == values.length ? 0 : /*values.flat()*/ validator.flatten(values)
        )
        .name(name)
        .build();
    }
  ...
  class Builder {
    ...
    size(s) {
      ...
      props.get(this).size = ItemFormat.isSizeable(props.get(this).format)
        ? size
        : undefined;
